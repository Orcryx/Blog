...
class CommentRepository
{
    ...
    {
        $comments = $this->databaseService->prepareAndExecute(
            'SELECT comment.commentId, comment.comment, comment.isValidated, comment.postId, comment.userId, user.nickname FROM comment JOIN user ON comment.userId = user.userId WHERE comment.postId = :postId AND comment.isValidated ="1" ',
            ['postId' => $postId]
        );
        return $comments;
    }
    ...
    public function getCommentById(int $commentId): object
    {
        $comment = $this->databaseService->prepareAndExecuteOne(
            'SELECT comment.commentId, comment.comment, comment.isValidated, comment.postId, comment.userId, user.nickname FROM comment JOIN user ON comment.userId = user.userId WHERE comment.commentId = :commentId',
            ['commentId' => $commentId]
        );
        return $comment;
    }
    ...
    {
        $comments = $this->databaseService->query(
            'SELECT comment.commentId, comment.comment, comment.isValidated, comment.postId, comment.userId, user.nickname FROM comment JOIN user ON comment.userId = user.userId WHERE comment.isValidated = "0" '
        );
        return $comments;
    }
